...
def scenario(feature_name, scenario_name, encoding="utf-8", example_converters=None, features_base_dir=None,scenario_to_execute=None):
    """Scenario decorator.
...
def _execute(scenario_object, scenario_name, module_scenarios, caller_locals, feature, **kwargs):
...
def scenarios(*feature_paths, **kwargs):
    ...
    scenario_to_execute = kwargs.get("scenario_to_execute", None)
    for feature in get_features(abs_feature_paths):
        if scenario_to_execute:
            scenario_object = feature.scenarios.get(scenario_to_execute)
            _execute(scenario_object=scenario_object, scenario_name=scenario_to_execute,
                     module_scenarios=module_scenarios, caller_locals=caller_locals, feature=feature, **kwargs)
            found = True
        else:
            for scenario_name, scenario_object in feature.scenarios.items():
                _execute(scenario_object=scenario_object, scenario_name=scenario_name,
                         module_scenarios=module_scenarios, caller_locals=caller_locals, feature=feature,
                         **kwargs)
                found = True
    if not found:
